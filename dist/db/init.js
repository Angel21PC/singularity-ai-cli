...
export function initDb() {
  ...
  `);
    ...
    try {
        const cols = db.prepare('PRAGMA table_info(TaskQueue)').all().map(c => c.name);
        if (!cols.includes('project_id'))
            db.exec('ALTER TABLE TaskQueue ADD COLUMN project_id TEXT');
        if (!cols.includes('prompt'))
            db.exec('ALTER TABLE TaskQueue ADD COLUMN prompt TEXT');
        if (!cols.includes('result'))
            db.exec('ALTER TABLE TaskQueue ADD COLUMN result TEXT');
        if (!cols.includes('created_at'))
            db.exec('ALTER TABLE TaskQueue ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP');
        if (!cols.includes('updated_at'))
            db.exec('ALTER TABLE TaskQueue ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP');
    }
    catch (e) {
        // non-fatal
    }
